...
def get_effectiveness(attacking_type, defending_types):
    """
    Calculate the effectiveness multiplier of an attacking type against a defending type or multiple types.
    For multiple defending types, the multipliers are multiplied together.
    """
    multiplier = 1.0
...
# Generate all triple types (unordered triplets)
triple_type_pairs = list(itertools.combinations(single_types, 3))

# Function to generate sorted type names
def generate_type_name(types):
    return '/'.join(sorted(types))

# Create lists of all types: single, dual, and triple
all_types = single_types.copy()

# Add dual types
all_types += [generate_type_name(pair) for pair in dual_type_pairs]

# Add triple types
all_types += [generate_type_name(triplet) for triplet in triple_type_pairs]

# Verify total number of types
num_types = len(all_types)  # 18 + 153 + 816 = 987
print(f"Total number of types: {num_types}")
...
# Initialize the type effectiveness matrix (987 x 987)
...
for atk_idx, atk_type in enumerate(all_types):
    ...
    for def_idx, def_type in enumerate(all_types):
        ...
        if len(atk_components) == 1:
            ...
            eff = max(eff1, eff2)
        elif len(atk_components) == 3:
            # Triple-type attacker: choose the maximum effectiveness among the three types
            eff_values = [get_effectiveness(atk, def_components) for atk in atk_components]
            eff = max(eff_values)
        else:
            # In case of unexpected number of types
            eff = 1.0
...
# Defense weight factor (increased to emphasize defense more)
defense_weight = 5.0  # 以前は2.0でした
...
print(f"{'Rank':<5} {'Type':<30} {'Score':<10}")
print("-" * 50)
for rank, type_name, score in ranked_types:
    print(f"{rank:<5} {type_name:<30} {score:<10.6f}")
